(function (root) {
  ...
  var CARD_META = {
    ...
    eveningFullBodyReset: { name:'Full-Body Reset', kicker:'Evening', number:'7', format:'Low-impact · bodyweight', summary:'7 movements · 40s work / 10s change' },
    postRunReset: { name:'Post-Run Reset', kicker:'Post-run', number:'', format:'Top-up + cooldown', summary:'Upper-body top-up, downshift, full stretch' }
  };
  ...
  function detailDefinition(workoutId) {
    ...
    if(workout.termination&&workout.termination.type==='timed-sequence') {
      var timedDescription=workout.description||(workout.transitionSeconds?'One continuous clock with a short transition between each exercise.':'One continuous clock. The next exercise starts immediately with no separate rest or transition time.');
      return {id:workoutId,name:workout.name,format:workout.subtitle||Math.round(workout.termination.durationSeconds/60)+' min continuous',description:timedDescription,exercises:workout.exercises.map(function(exercise){return {name:exercise.name,target:exercise.durationSeconds+' sec',note:exercise.note||''};})};
    }
    if(workout.termination&&workout.termination.type==='fixed-sequence') {
      var sequenceDescription=workout.description||'A single linear pass through the sequence below. No rounds, no repeats.';
      return {id:workoutId,name:workout.name,format:workout.subtitle||'Single pass',description:sequenceDescription,exercises:workout.exercises.map(function(exercise){return {name:exercise.name,target:exercise.reps!=null?exercise.reps+' reps':exercise.durationSeconds+' sec',note:exercise.note||''};})};
    }
  ...
  function startSelectedWorkout(){if(selectedWorkoutId==='sevenMinute')startSevenMinuteWorkout();else if(selectedWorkoutId&&FIXED_WORKOUTS[selectedWorkoutId]&&FIXED_WORKOUTS[selectedWorkoutId].termination.type==='timed-sequence')startTimedSequenceWorkout(selectedWorkoutId);else if(selectedWorkoutId&&FIXED_WORKOUTS[selectedWorkoutId]&&FIXED_WORKOUTS[selectedWorkoutId].termination.type==='fixed-sequence')startFixedSequenceWorkout(selectedWorkoutId);else if(selectedWorkoutId)startFixedWorkout(selectedWorkoutId);}
  ...
  function timedSequenceList(workout){var transitionSeconds=workout.transitionSeconds||0;return workout.exercises.map(function(exercise,index){return {name:exercise.name,work:exercise.durationSeconds,rest:index<workout.exercises.length-1?transitionSeconds:0};});}
  function startTimedSequenceWorkout(workoutId){
    ...
    var transitionSeconds=workout.transitionSeconds||0;
    var total=workout.exercises.reduce(function(sum,exercise){return sum+exercise.durationSeconds;},0)+transitionSeconds*Math.max(0,workout.exercises.length-1);if(total!==workout.termination.durationSeconds)throw new Error('Timed sequence durations must equal the overall workout duration.');
  ...
  function enterTimedExercise(){var ex=state.list[state.idx];state.phase='work';state.remaining=ex.work;updateProgressCount();document.getElementById('exerciseName').textContent=ex.name;document.getElementById('phaseLabel').textContent='WORK';setRingColor('work');document.getElementById('repsNum').classList.add('hidden');var next=state.idx<state.list.length-1?state.list[state.idx+1]:null;document.getElementById('nextPreview').textContent=next?'Next: '+next.name:'Last one!';beep(880);speak(ex.name+'. '+ex.work+' seconds');runTimer(ex.work,enterTimedTransition);}
  function enterTimedTransition(){var ex=state.list[state.idx];if(state.idx>=state.list.length-1||!ex.rest){advanceTimedSequence();return;}state.phase='rest';state.remaining=ex.rest;updateProgressCount();var next=state.list[state.idx+1];document.getElementById('exerciseName').textContent='Get ready';document.getElementById('phaseLabel').textContent='REST';document.getElementById('repsNum').classList.add('hidden');setRingColor('rest');document.getElementById('nextPreview').textContent='Next: '+next.name;beep(440);speak(next.name);runTimer(ex.rest,advanceTimedSequence);}
  function advanceTimedSequence(){if(state.idx>=state.list.length-1){finishTimedSequence();return;}state.idx++;enterTimedExercise();}
  function finishTimedSequence(){var workout=timedSequence&&timedSequence.workout;stopTimer();state.running=false;state.doneDestination='workouts';releaseWake();document.getElementById('doneTitle').textContent=(workout?workout.name:'Workout')+' Complete';document.getElementById('doneSub').textContent=(workout&&workout.completionMessage)||'Nice work.';document.getElementById('doneStats').classList.add('hidden');document.getElementById('generatedDoneStats').classList.add('hidden');document.getElementById('amrapDoneStats').classList.add('hidden');document.getElementById('doneBest').classList.add('hidden');document.getElementById('doneBtn').textContent='Back to workouts';showScreen('done');speak('Workout complete. Nice work.');timedSequence=null;}
  function skipTimedPhase(){clearInterval(state.timer);state.paused=false;document.getElementById('pauseBtn').textContent='Pause';if(state.phase==='ready')enterTimedExercise();else if(state.phase==='work')enterTimedTransition();else if(state.phase==='rest')advanceTimedSequence();}

  // A fixed-sequence workout is a single linear pass mixing rep-based movements (completed by
  // the user tapping "Complete", same as the reps-driven fixed workouts) with timed movements
  // (an auto countdown, same as the timed-sequence workouts). No rounds, no repeats.
  function fixedSequenceList(workout){var defaultTransition=workout.transitionSeconds||0;return workout.exercises.map(function(exercise,index){var isLast=index===workout.exercises.length-1,transition=exercise.transitionAfter!=null?exercise.transitionAfter:defaultTransition;return {name:exercise.name,reps:exercise.reps!=null?exercise.reps:null,work:exercise.durationSeconds!=null?exercise.durationSeconds:null,rest:isLast?0:transition};});}
  function startFixedSequenceWorkout(workoutId){
    var workout=FIXED_WORKOUTS[workoutId];if(!workout||!workout.exercises||workout.termination.type!=='fixed-sequence')return;
    fixedSequence={workout:workout,index:0};state.key='fixedSequence';state.list=fixedSequenceList(workout);state.idx=0;state.paused=false;state.running=true;state.doneDestination='workouts';document.getElementById('pauseBtn').textContent='Pause';document.getElementById('routineName').textContent=workout.name;showScreen('player');requestWake();enterFixedSequenceReady();
  }
  function enterFixedSequenceReady(){state.phase='ready';state.remaining=3;updateProgressCount();var first=state.list[0];document.getElementById('exerciseName').textContent='Get ready';document.getElementById('nextPreview').textContent='First up: '+first.name;setRingColor('rest');document.getElementById('phaseLabel').textContent='GET READY';document.getElementById('repsNum').classList.add('hidden');speak('Get ready. '+first.name);runTimer(3,enterFixedSequenceStep);}
  function enterFixedSequenceStep(){
    var ex=state.list[state.idx];updateProgressCount();document.getElementById('exerciseName').textContent=ex.name;setRingColor('work');var next=state.idx<state.list.length-1?state.list[state.idx+1]:null;document.getElementById('nextPreview').textContent=next?'Next: '+next.name:'Last one!';beep(880);
    if(ex.reps!=null){
      state.phase='reps';document.getElementById('phaseLabel').textContent='COMPLETE';document.getElementById('timerNum').textContent='';var repsEl=document.getElementById('repsNum');repsEl.classList.remove('hidden');repsEl.textContent='x'+ex.reps+' reps';document.getElementById('ringFg').style.strokeDashoffset='0';speak(ex.name+'. '+ex.reps+' reps. Tap complete when done.');
    } else {
      state.phase='work';state.remaining=ex.work;document.getElementById('phaseLabel').textContent='WORK';document.getElementById('repsNum').classList.add('hidden');speak(ex.name+'. '+ex.work+' seconds');runTimer(ex.work,enterFixedSequenceTransition);
    }
  }
  function enterFixedSequenceTransition(){var ex=state.list[state.idx];if(state.idx>=state.list.length-1||!ex.rest){advanceFixedSequence();return;}state.phase='rest';state.remaining=ex.rest;updateProgressCount();var next=state.list[state.idx+1];document.getElementById('exerciseName').textContent='Get ready';document.getElementById('phaseLabel').textContent='REST';document.getElementById('repsNum').classList.add('hidden');setRingColor('rest');document.getElementById('nextPreview').textContent='Next: '+next.name;beep(440);speak(next.name);runTimer(ex.rest,advanceFixedSequence);}
  function advanceFixedSequence(){if(state.idx>=state.list.length-1){finishFixedSequence();return;}state.idx++;enterFixedSequenceStep();}
  function finishFixedSequence(){var workout=fixedSequence&&fixedSequence.workout;stopTimer();state.running=false;state.doneDestination='workouts';releaseWake();document.getElementById('doneTitle').textContent=(workout?workout.name:'Workout')+' Complete';document.getElementById('doneSub').textContent=(workout&&workout.completionMessage)||'Nice work.';document.getElementById('doneStats').classList.add('hidden');document.getElementById('generatedDoneStats').classList.add('hidden');document.getElementById('amrapDoneStats').classList.add('hidden');document.getElementById('doneBest').classList.add('hidden');document.getElementById('doneBtn').textContent='Back to workouts';showScreen('done');speak('Workout complete. Nice work.');fixedSequence=null;}
  function skipFixedSequence(){clearInterval(state.timer);state.paused=false;document.getElementById('pauseBtn').textContent='Pause';if(state.phase==='ready')enterFixedSequenceStep();else if(state.phase==='work'||state.phase==='reps')enterFixedSequenceTransition();else if(state.phase==='rest')advanceFixedSequence();}
  ...
  function cleanCatalogue(){var list=document.querySelector('.workout-list');if(!list)return;var buttons=list.querySelectorAll('.workout-launch');if(buttons.length>=3){rewriteCard(buttons[0],'sevenMinute');rewriteCard(buttons[1],'cindy');rewriteCard(buttons[2],'workout300');if(!list.querySelector('[data-workout-id="abBlast"]')){addWorkoutCard(list,'abBlast');var added=list.lastElementChild;added.setAttribute('data-workout-id','abBlast');}if(!list.querySelector('[data-workout-id="eveningFullBodyReset"]')){addWorkoutCard(list,'eveningFullBodyReset');var addedReset=list.lastElementChild;addedReset.setAttribute('data-workout-id','eveningFullBodyReset');}if(!list.querySelector('[data-workout-id="postRunReset"]')){addWorkoutCard(list,'postRunReset');var addedPostRun=list.lastElementChild;addedPostRun.setAttribute('data-workout-id','postRunReset');}}['.workout-detail','.review-toggle','#workoutReview'].forEach(function(selector){var element=document.querySelector(selector);if(element)element.remove();});var legacyBest=document.querySelector('.best-strip');if(legacyBest)legacyBest.classList.add('hidden');}
  ...
  function patchTimedSequenceControls(){baseSkipPhase=root.skipPhase;if(typeof baseSkipPhase==='function')root.skipPhase=function(){if(state.key==='fixedTimedSequence')skipTimedPhase();else if(state.key==='fixedSequence')skipFixedSequence();else baseSkipPhase();};}
  ...
  root.GarageFitWorkoutUI={cardMeta:CARD_META,detailDefinition:detailDefinition,openWorkoutDetail:openWorkoutDetail,startSelectedWorkout:startSelectedWorkout,startTimedSequenceWorkout:startTimedSequenceWorkout,startFixedSequenceWorkout:startFixedSequenceWorkout,skipFixedSequence:skipFixedSequence,install:install};
